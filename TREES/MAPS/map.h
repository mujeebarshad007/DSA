...
public:
    ...
    T &operator[](const key_type &key)
    {
        ...
        if (it != end()) // key exists
        {
            return it.ptr->data.second;
        }
        else // key does not exist, insert default
        {
            std::pair<iterator, bool> res = insert({key, T()});
    ...
    iterator erase(iterator pos)
    {
        ...
        mnode<key_type, T> *rebalance = to_del->parent; // takes the parent of deleted node and balance it.
        ...
        else
        {
            ...
            // fix parent pointer of successor's child
            if (succ_r != H)
